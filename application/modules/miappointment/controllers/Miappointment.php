...
class Miappointment extends MY_Controller {
    ...
    function index() {
        ...
        //$data['dignostiData'] = $this->miappointment->getDiagnostic();
    ...
    function detailBloodBank($bloodBankId = '') {
        $data = array();
        $data['bloodBankData'] = $this->Miappointment->fetchbloodBankData($bloodBankId);
    ...
    function SaveBloodbank() {
        ...
        if ($this->bf_form_validation->run($this) === FALSE) {
            ...
            $bloodbank_usersId = $this->Miappointment->insertBloodBankUser($bloodBankInsert);
            if ($bloodbank_usersId) {
                ...
                $this->Miappointment->insertUsersRoles($insertusersRoles);
                ...
                $bloodBankId = $this->Miappointment->insertBloodBank($insertData);
            }
    ...
    function check_email() {
        ...
        $email = $this->Miappointment->fetchEmail($users_email, $user_table_id);
        //echo $email;exit;
        echo $email;
        exit;
    }
    ...
    function saveDetailBloodBank($bloodBankId) {
        ...
        if ($this->bf_form_validation->run() === FALSE) {
            ...
            $response = '';
            $response = $this->Miappointment->UpdateTableData($updateBloodBank, $where, 'qyura_bloodBank');
            if ($response) {
                ...
                $response = $this->Miappointment->UpdateTableData($updateUserdata, $whereUser, 'qyura_users');
                if ($response) {
    ...
    function updatePassword() {
        ...
        /* if($encrypted != $existingPassword){
          ...
          $this->Miappointment->UpdateTableData($updateBloodBank,$where,'qyura_users');
        ...
        $return = $this->Miappointment->UpdateTableData($updateBloodBank, $where, 'qyura_users');
    ...
    function editUploadImage() {
        ...
        if ($_POST['avatar_file']['name']) {
            ...
            if (empty($original_imagesname)) {
                ...
                $response = $this->Miappointment->UpdateTableData($option, $where, 'qyura_bloodBank');
                if ($response) {
    ...
    function getUpdateAvtar($bloodBankId){
        if(!empty($bloodBankId)){
          $data = $this->Miappointment->fetchbloodBankData($bloodBankId);
