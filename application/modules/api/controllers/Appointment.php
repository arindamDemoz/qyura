...
THEN CASE usersfamily_gender WHEN 0 THEN 'Male' WHEN 1 THEN 'Female' WHEN 3 THEN 'Other' END  ELSE CASE patientDetails_gender WHEN 'M' THEN 'Male' WHEN 'F' THEN 'Female' WHEN 'O' THEN 'Other' END END AS `userGender`, `qyura_users`.`users_mobile` AS `usersMobile`, CASE WHEN (`qyura_quotations`.`quotation_familyId` <> 0 ) THEN qyura_usersFamily.usersfamily_age ELSE (FROM_UNIXTIME('{$now}', '%Y') - FROM_UNIXTIME(qyura_patientDetails.patientDetails_dob, '%Y')) END AS `userAge`, `transactionInfo`.`payment_method` AS `paymentMethod`, '' AS `remark`, `qyura_diagnosticsCat`.`diagnosticsCat_catName` AS `diagCatName`, '' AS `speciality`, 'Diagnostic' as `type`, (CASE WHEN(quotation_dateTime > CURRENT_TIMESTAMP ) THEN 'Upcoming' ELSE 'Completed' END) as `upcomingStatus`, CASE WHEN (`qyura_hospital`.`hospital_usersId` <> 0 ) THEN qyura_hospital.hospital_usersId ELSE qyura_diagnostic.diagnostic_usersId END AS `miId`
...
';
            $sql3 = "SELECT  `qyura_users`.`users_username` as title, concat(DATE_FORMAT(FROM_UNIXTIME(`qyura_doctorAppointment`.`doctorAppointment_date`),'%Y-%m-%d'),' ',doctorAvailabilitySession_start)  as `dateTime`, CASE WHEN (`qyura_doctorAppointment`.`doctorAppointment_docType` = 1 ) THEN `qyura_hospital`.`hospital_address` WHEN (`qyura_doctorAppointment`.`doctorAppointment_docType` = 2 ) THEN `qyura_diagnostic`.`diagnostic_address` ELSE `qyura_doctors`.`doctor_addr` END AS `address`,`qyura_doctorAppointment`.`doctorAppointment_unqId` AS `orderId`, CASE qyura_doctorAppointment.doctorAppointment_status WHEN '0' THEN 'pending' WHEN '1' THEN 'confirmed' ELSE NULL END AS `bookingStatus`, CASE transactionInfo.payment_status WHEN '1' THEN 'Success' WHEN 4 THEN 'Aborted' WHEN 5 THEN 'Failure' ELSE NULL END AS `paymentStatus`, CASE WHEN (qyura_doctorAppointment.doctorAppointment_memberId <> 0 ) THEN qyura_usersFamily.usersfamily_name ELSE qyura_patientDetails.patientDetails_patientName END AS `userName`, CASE WHEN (qyura_doctorAppointment.doctorAppointment_memberId <> 0 )  THEN CASE usersfamily_gender WHEN '0' THEN 'Male' WHEN '1' THEN 'Female' WHEN 3 THEN 'Other' END  ELSE CASE patientDetails_gender WHEN '0' THEN 'Male' WHEN '1' THEN 'Female' WHEN 'O' THEN 'Other' END END AS `userGender`, `qyura_users`.`users_mobile` AS `usersMobile`, CASE WHEN (`qyura_doctorAppointment`.`doctorAppointment_memberId` <> 0 ) THEN qyura_usersFamily.usersfamily_age ELSE (FROM_UNIXTIME('{$now}', '%Y') - FROM_UNIXTIME(qyura_patientDetails.patientDetails_dob, '%Y')) END AS `userAge`, `transactionInfo`.`payment_method` AS `paymentMethod`, `qyura_doctorAppointment`.`doctorAppointment_ptRmk` AS `remark`, '' AS `diagCatName`, `qyura_specialities`.`specialities_name` AS `speciality`, 'Consultation' as `type`, CASE WHEN (concat(DATE_FORMAT(FROM_UNIXTIME(`qyura_doctorAppointment`.`doctorAppointment_date`),'%Y-%m-%d'),' ',doctorAvailabilitySession_start) > CURRENT_TIMESTAMP ) THEN 'Upcoming' ELSE 'Completed' END as `upcomingStatus`, CASE WHEN (`qyura_hospital`.`hospital_usersId` <> 0 ) THEN qyura_hospital.
hospital_usersId ELSE qyura_diagnostic.diagnostic_usersId END AS `miId`  FROM `qyura_doctorAppointment`
...
LEFT JOIN `qyura_doctors` ON `qyura_doctors`.`doctors_userId` = `qyura_doctorAppointment`.`doctorAppointment_doctorParentId`
...
AND `qyura_doctorAppointment`.`doctorAppointment_date` <> 0";
            ...
            $sql = $sql1 . " " . $sql2 . " " . $sql3;
// (CASE WHEN ((FROM_UNIXTIME(qyura_doctorAppointment.doctorAppointment_date, '%Y %M %D'),'',doctorAvailabilitySession_start) > CURRENT_TIMESTAMP )
            $queryResult = $this->db->query($sql)->result();

//            dump($this->db->last_query()); die();

            $colName = array("title", "dateTime", "address", "orderId", "bookingStatus", "paymentStatus", "userName", "userGender", "usersMobile", "userAge", "paymentMethod", "remark", "diagCatName", "speciality", "type", "upcomingStatus","miId");
