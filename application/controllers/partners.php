    ...
    function aerobics() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $closingHours = $this->input->post('closingHours');

                $times = array();
                $timeArrayOpening = array();
                $timeArrayClosing = array();
                $arrayFilterOpening = array_filter($openingHours);
                $arrayFilterClosing = array_filter($closingHours);
                $k = 0;

                if (count($days) && count($openingHours) && !empty($days) && !empty($openingHours)) {

                    foreach ($days as $daykey) {
                        $timeArrayOpening[$daykey] = $arrayFilterOpening[$k];
                        $timeArrayClosing[$daykey] = $arrayFilterClosing[$k];
                        $k++;
                    }
                }
                ...
                $arrayDiscFilter = array_filter($discAmount);
                $j = 0;
                $m = 0;
                if (count($chargeType) && count($chargeAmount) && !empty($chargeType) && !empty($chargeAmount)) {
                    ...
                    for ($i = 0; $i < count($chargeType); $i++) {
                        ...
                        $dAmount[$chargeType[$i]] = $amtDiscArray[$i];
                    }
                }
                ...
                $arrayData = array(
                    ...
                    'discAmount' => serialize($dAmount),
                    'currency' => $this->input->post('currency'),
                    'centerType' => $this->input->post('centerType'),
                    'days' => serialize($days),
                    'openingHours' => serialize($timeArrayOpening),
                    'closingHours' => serialize($timeArrayClosing),
                    'womenDays' => serialize($this->input->post('womenDays')),
                    'createAt' => date('Y-m-d'),
                    'updateFlag' => 1,
                    'tableName' => 'aerobics',
...
//                $result = $this->commonInsertData($arrayData,$aeroId,'aerobics');
                ...
                if ($result) {
                    ...
                    redirect(site_url("partners/vendorCustomView/$vendorId/aerobics"));
//                    $head = $this->header();
//                    $update = 0;
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                if ($result) {
                    ...
                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/crossfunctional"));
...
//                    'vehicle' => $this->input->post('vehicle'),
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'fitnessstudio');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>');
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/fitnessstudio"));
    ...
    function dance() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'dance');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/dance"));
    ...
    function dietitiannutritionist() {
        ...
        $Availability_option = array(
            'fkVendorId' => $this->input->post('fkVendorId'),
            'tableName' => 'dietitiannutritionist'
        );
        $this->checkAvailability($Availability_option);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'dietitiannutritionist');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/dietitiannutritionist"));
...
//                    'vehicle' => $this->input->post('vehicle'),
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'gym');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>');
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/gym"));
    ...
    function karatemartialart() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'karatemartialart');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/karatemartialart"));
    ...
    function kickboxing() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $kickId = array('kickId' => $this->input->post('kickId'));
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'kickboxing');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/kickboxing"));
    ...
    function massages() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'massages');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/massages"));
    ...
    function pilates() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'pilates');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/pilates"));
    ...
    function swimming() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'swimming');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/swimming"));
    ...
    function spinning() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'spinning');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/spinning"));
    ...
    function yoga() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'yoga');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/yoga"));
    ...
    function zumba() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'zumba');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/zumba"));
    ...
    function others() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST") {
                ...
                $result = $this->main_content_model->customInsertServices($arrayData, 'others');
                if ($result) {

                    $this->session->set_flashdata('messages', '<strong><span style="color:green">Your Information submited successfully.Wait for superadmin approval</span></strong>'); 
                    $vendorId = $this->session->userdata('vendorId');
                    redirect(site_url("partners/vendorCustomView/$vendorId/others"));
